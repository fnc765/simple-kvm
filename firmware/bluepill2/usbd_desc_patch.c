/**
  ******************************************************************************
  * @file    usbd_desc_patch.c
  * @brief   Patch: USB descriptor override for Logitech Unifying Receiver emulation
  *
...
__ALIGN_BEGIN uint8_t USBD_Class_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END = {
  ...
  0x00,                       /* iSerial = 0 (no serial number string) */
...
/**
  * @brief  PATCHED: Returns empty serial number string descriptor (iSerial=0 equivalent).
  * @param  speed: Current device speed
  * @param  length: Pointer to data length variable
  * @retval Pointer to descriptor buffer
  */
uint8_t *USBD_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  /* iSerial=0: no serial number string */
  *length = 0U;
