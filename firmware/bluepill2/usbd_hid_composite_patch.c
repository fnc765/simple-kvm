/**
  ******************************************************************************
  * @file    usbd_hid_composite_patch.c
  * @brief   Patch: HID Composite implementation for Logitech Unifying Receiver
  *
  * Changes from original usbd_hid_composite.c:
  *   (a) Config descriptor interface order: Keyboard→Mouse (not Mouse→Keyboard)
  *   (b) Mouse report descriptor: expanded to 76 bytes (5-byte report with wheel_h)
  *   (c) Keyboard report descriptor: expanded to 61 bytes (Boot Keyboard + LED output)
  *
  * Original source:
  *   ~/.platformio/packages/framework-arduinoststm32/libraries/USBDevice/src/hid/usbd_hid_composite.c
  ******************************************************************************
  */
...
/* Includes ------------------------------------------------------------------*/
/* Use patched header with swapped interface numbers and updated sizes */
...
#ifndef USE_USBD_COMPOSITE
/*
 * PATCHED: Keyboard interface first (bInterfaceNumber=0x00),
 *          Mouse interface second (bInterfaceNumber=0x01)
 *          (Logitech C52B: Keyboard is interface 0, Mouse is interface 1)
 */
...
__ALIGN_BEGIN static uint8_t USBD_HID_CfgFSDesc[USB_COMPOSITE_HID_CONFIG_DESC_SIZ] __ALIGN_END = {
  ...
  *********************************************************************/
  ...
  0x00,                                               /* bInterfaceNumber: Keyboard = 0 */
...
/*
 * PATCHED: 5-byte mouse report descriptor (76 bytes)
 * Added horizontal wheel axis for full 5-byte report support.
 * Report format: [button:1][X:1][Y:1][wheel_v:1][wheel_h:1]
 */
__ALIGN_BEGIN static uint8_t HID_MOUSE_ReportDesc[HID_MOUSE_REPORT_DESC_SIZE] __ALIGN_END = {
  ...
  0x09, 0x38,        /*     Usage (Wheel) - horizontal scroll  */
  0x15, 0x81,        /*     Logical Minimum (-127)             */
  0x25, 0x7F,        /*     Logical Maximum (127)              */
  0x75, 0x08,        /*     Report Size (8)                    */
  0x95, 0x04,        /*     Report Count (4) - X,Y,WheelV,WheelH */
...
/*
 * PATCHED: Boot Keyboard report descriptor with LED output (61 bytes)
 *
 * Input report (8 bytes):  modifier(1) + reserved(1) + keys[6]
 * Output report (1 byte):  LEDs (NumLock/CapsLock/ScrollLock) + padding
 */
__ALIGN_BEGIN static uint8_t HID_KEYBOARD_ReportDesc[HID_KEYBOARD_REPORT_DESC_SIZE]  __ALIGN_END = {
  ...
  /* Output report: Keyboard LEDs (NumLock, CapsLock, ScrollLock) */
                             ...
                             uint8_t cfgidx)
{
  ...
  /* Open EP IN */
  (void)USBD_LL_OpenEP(pdev, HIDMInEpAdd, USBD_EP_TYPE_INTR,
                       HID_MOUSE_EPIN_SIZE);
