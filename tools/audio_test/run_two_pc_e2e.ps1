...
$runner = Join-Path $repoRoot "tools\audio_test\two_pc_e2e.py"

# Mandatory pre-execution reminder. The Python controller and the remote
# target independently repeat this check before touching either device.
& $PythonExe $policy `
    --name "two-PC BP1/BP2 audio E2E" `
    --stage $Stage `
    --duration $Seconds
