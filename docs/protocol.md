...
- 単発クリックは同じ座標の `buttons=0 → buttons=<押下> → buttons=0` の順で送ります。
- ドラッグ終了は、終点へ押下状態のまま移動してから同じ座標で解放します。
- ホスト側では、ボタン状態が変わらない移動は latest-wins（未送信の最新1件）として扱い、押下・解放シーケンスは通常送信キューの混雑にかかわらず FIFO 順で保持します。
- BP2 側でも USB endpoint が `BUSY` の間はレポートを保持し、同一ボタン状態の移動だけを最新座標へ集約します。
- heartbeat を含む有効 UART パケットが 2.5 秒途絶えた状態で絶対ボタンが押下中なら、BP2 は最後の座標で解放レポートを生成します。
